...
/**
 * <p>
 * A stateless helper that performs dynamic class loading and instantiation with
 * support for invoking Singleton-retrieval methods of classes if detected.
 * </p>
 * <p>
 * Before invoking <code>someClass.newInstance()</code> the loaded class is
 * inspected for methods, that are public, static, parameterless, contain the
 * substring "intance" in their name (case-insensitive) and return an instance
 * of the same type.
 * </p>
 * <p>
 ...
 * triggered by <code>newInstance()</code> (which then should be private if it
 * is really a singleton.
 * </p>
 * <p>
 * This class is in turn a singleton as it only contains procedural code. For
 * possible desireable future configuration requirements, the single public
 * method is not declared static but bound to the singleton instance.
 * </p>
 * 
...
public final class SingletonLoader {
  ...
  public static SingletonLoader getInstance() {
    if (instance == null) {
      synchronized (SingletonLoader.class) {
   ...
   * specified by the given fully qualified class name with singleton retrieval
   * support. Delegates to {@link #newInstance(Class)}.
   * 
   */
  ...
  /**
   * Dynamic instantiation of the given type with singleton retrieval support as
            ...
            if (m.getReturnType() == c) {
              ...
              if (m.getName().toLowerCase().indexOf("instance") != -1) {
                try {
                  ...
                  // This will not happen (only in applet context perhaps or
                  // with some
                  // SecurityManager):
