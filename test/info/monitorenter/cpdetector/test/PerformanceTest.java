...
public class PerformanceTest extends ACmdLineArgsInheritor {
    ...
    /**
     * Amount of detections repeatedly performed on every document. A high number minimizes the time measurement error
     * (but causes the test to take longer).
     */
    ...
    /**
     * All documents that were detected to have a different codepage than the human - verified one.
     */
    ...
    public PerformanceTest() {
        ...
        this.addCmdLineOption("iterations", new CmdLineParser.Option.IntegerOption('i',
                "iterations"));
    ...
    public void parseArgs(String[] cmdLineArgs) throws Exception {
        ...
        if (detectorOption != null) {
            ...
            if (detectors.length == 0) {
                ...
                msg
                        .append("You specified the codepage detector argument \"-c\" but ommited any comma-separated fully qualified class-name.");
    ...
    public final void process() throws Exception {
        ...
        System.out.println("Processed " + this.filecount + " * " + this.iterations + " = "
                + (this.filecount * this.iterations) + " in "
                + this.stopWatch.getPureMilliSeconds() + " ms.");
        ...
        if (this.unEqual.size() > 0) {
            System.out.println(this.unEqual.size()
                    + " documents differ between detected an human verified:");
    ...
    private void process(File document) throws Exception {
        ...
        try {
            // this should throw "already closed":
            cache.close();
        ...
        if (!autoClosed) {
            throw new RuntimeException("InputStream to " + document.getAbsolutePath()
                    + " should be closed after detection.");
        }
        ...
        if (verified != detected) {
            if (!FileUtil.isAllASCII(document)) {
                ...
                if ((detected != UnknownCharset.getInstance())
                        && !(detected.getClass() == UnsupportedCharset.class)) {
    ...
    protected void usage() {
        ...
        tmp
                .append("  -i              : Iterations per document. A high value decreases time measurement error \n");
        tmp.append("                     but causes the test to take longer.\n");
        tmp
                .append("  -c              : Semicolon-separated list of fully qualified classnames. \n");
        tmp
